...
def CheckPythonPaths(verbose):
    ...
    key = win32api.RegOpenKey(regutil.GetRootKey(), regutil.BuildDefaultPythonKey() + "\\PythonPath", 0,
                              win32con.KEY_READ)
    try:
        keyNo = 0
        while 1:
...
def CheckHelpFiles(verbose):
    ...
    try:
        key = win32api.RegOpenKey(regutil.GetRootKey(), regutil.BuildDefaultPythonKey() + "\\Help", 0,
                                  win32con.KEY_READ)
    except win32api.error as exc:
        import winerror
